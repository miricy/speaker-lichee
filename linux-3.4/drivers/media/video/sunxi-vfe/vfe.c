...
static int isp_resource_request(struct vfe_dev *dev)
{
	...
	for(i=0; i < dev->dev_qty; i++) {
		if(dev->ccm_cfg[i]->is_isp_used && dev->ccm_cfg[i]->is_bayer_raw) {
			dev->isp_lut_tbl_buf_mm[i].size = ISP_LUT_LENS_GAMMA_MEM_SIZE;
			ret = os_mem_alloc(&dev->isp_lut_tbl_buf_mm[i],VFE_NOT_ADDR);
			if(!ret) {
		...
		if(dev->ccm_cfg[i]->is_isp_used) {
			dev->isp_drc_tbl_buf_mm[i].size = ISP_DRC_MEM_SIZE;
			ret = os_mem_alloc(&dev->isp_drc_tbl_buf_mm[i], VFE_NOT_ADDR);
			if(!ret) {
				dev->isp_tbl_addr[i].isp_drc_tbl_paddr = (void*)(dev->isp_drc_tbl_buf_mm[i].phy_addr);
				dev->isp_tbl_addr[i].isp_drc_tbl_dma_addr = (void*)(dev->isp_drc_tbl_buf_mm[i].dma_addr);
				dev->isp_tbl_addr[i].isp_drc_tbl_vaddr = (void*)(dev->isp_drc_tbl_buf_mm[i].vir_addr);;
	...
	if(isp_used_flag) {
		for(i=0; i < MAX_ISP_STAT_BUF; i++) {
			...
			ret = os_mem_alloc(&dev->isp_stat_buf_mm[i], VFE_NOT_ADDR);
			if(!ret) {
...
static int vfe_clk_get(struct vfe_dev *dev)
{
	...
	//for A80
#ifdef CSI_MISC_CLK
	dev->clock.vfe_misc_clk = os_clk_get(NULL, CSI_MISC_CLK);
#endif
...
static int isp_set_capture_flash(struct vfe_dev *dev)
{
	...
	if(dev->isp_gen_set_pt->take_pic_start_cnt == 5 + dev->isp_gen_set_pt->isp_ini_cfg.isp_tunning_settings.flash_delay_frame)
	{
		vfe_dbg(0,"close flash when nigth mode\n");
		io_set_flash_ctrl(dev->sd, SW_CTRL_FLASH_OFF, dev->fl_dev_info);
		dev->isp_gen_set_pt->exp_settings.tbl_cnt = CLIP(dev->isp_gen_set_pt->exp_settings.expect_tbl_cnt,
              ...
              enum v4l2_field field)
{
	...
	vb->boff = videobuf_to_dma_contig(vb) - CPU_DRAM_PADDR_ORG + HW_DMA_OFFSET;
...
//      printk("dev->mipi_fmt.packet_fmt[%d] = %x\n",i,dev->mipi_fmt.packet_fmt[i]);
  ...
  if(dev->is_isp_used) {
    ...
    dev->buf_byte_size = bsp_isp_set_size(isp_fmt,&ob_black_size,&isp_size[MAIN_CH],&isp_size[ROT_CH],&ob_start,&isp_size[SUB_CH]);
    vfe_print("dev->buf_byte_size = %d\n",dev->buf_byte_size);    
  } else {
  ...
  if(dev->is_isp_used && dev->is_bayer_raw) {
	...
	case V4L2_CID_B_GAIN: 
      ...
      case V4L2_CID_AUTOGAIN:
        return v4l2_ctrl_query_fill(qc, 0, 1, 1, 1);
      case V4L2_CID_GAIN: 
        return v4l2_ctrl_query_fill(qc, 0, 0xffffffff, 1, 0);
      case V4L2_CID_VFLIP:
       ...
       struct v4l2_control *ctrl)
{
  ...
  if(dev->is_isp_used && dev->is_bayer_raw) {
    switch (ctrl->id) {
      //V4L2_CID_BASE
      case V4L2_CID_BRIGHTNESS:
	ctrl->value = dev->isp_gen_set_pt->ae_lum;
        break;
      case V4L2_CID_CONTRAST:
	...
	case V4L2_CID_FOCUS_LENGTH:
	  ...
	  case V4L2_CID_AUTO_WHITE_BALANCE:
		...
		break;
	  case V4L2_CID_EXPOSURE:
	  	ctrl->value = v4l2_subdev_call(dev->sd,core,g_ctrl,ctrl);
		break;
	  case V4L2_CID_AUTOGAIN:
		ctrl->value = dev->ctrl_para.auto_gain;
		break;
	  case V4L2_CID_GAIN: 
	  	ctrl->value = CLIP(dev->isp_3a_result_pt->exp_analog_gain + 
	  				(dev->isp_gen_set_pt->sharp_cfg_to_hal[0]/*min*/ << 24) + (dev->isp_gen_set_pt->sharp_cfg_to_hal[1]/*level*/ << 12) ,0,0xffffffff);
		return ctrl->value;
	case V4L2_CID_R_GAIN:	
...
static int vfe_request_pin(struct vfe_dev *dev)
{
	int ret = 0;

#ifdef VFE_GPIO  
	dev->pctrl = devm_pinctrl_get_select_default(&dev->pdev->dev);
	...
	//writel((volatile void*)((readl((volatile void __iomem*)(GPIO_REGS_VBASE+0x24)) & 0xff00ffff) | 0x220000),\
	//														(volatile void __iomem*)(GPIO_REGS_VBASE+0x24));
...
static int vfe_resource_request(struct platform_device *pdev ,struct vfe_dev *dev)
{
...
#ifdef SUNXI_MEM
			regs = sunxi_map_kernel(res->start, resource_size(res));
#else
			regs = IO_ADDRESS(res->start);
#endif
			if (!regs) {
	...
	if(isp_va_flag == 0)
	{
		os_mem_alloc(&dev->isp_load_reg_mm, VFE_NOT_ADDR);
		os_mem_alloc(&dev->isp_save_reg_mm, VFE_NOT_ADDR);
		isp_load_va_addr = (unsigned int)dev->isp_load_reg_mm.phy_addr ;
		isp_save_va_addr = (unsigned int)dev->isp_save_reg_mm.phy_addr ;
		isp_va_flag = 1;
		isp_va_alloc_sel = dev->vip_sel;
	}
	else
	{
		os_mem_alloc(&dev->isp_load_reg_mm, isp_load_va_addr);  
		os_mem_alloc(&dev->isp_save_reg_mm, isp_save_va_addr);
	}
...
{  
	...
	if(dev->regs.csi_regs)
		sunxi_unmap_kernel(dev->regs.csi_regs);
	if(dev->regs.isp_regs && isp_va_flag)
		sunxi_unmap_kernel(dev->regs.isp_regs);
#endif
...
static int vfe_probe(struct platform_device *pdev)
{
	...
	vfe_print("pdev->id = %u\n",pdev->id);
	vfe_print("dev->mipi_sel = %u\n",pdata->mipi_sel);
	vfe_print("dev->vip_sel = %u\n",pdata->vip_sel);
	vfe_print("dev->isp_sel = %u\n",pdata->isp_sel);

	//to cheat the pinctrl
	dev_set_name(&dev->pdev->dev,"csi%u",dev->id);
	...
	dev_set_name(&dev->pdev->dev,"sunxi_vfe.%u",dev->id);
...
static int __init vfe_init(void)
{
	...
	for(i=0; i<MAX_VFE_INPUT; i++) {
		...
		if (ret)
			vfe_err("platform device %u register failed\n",i);
		}
	}
...
static void __exit vfe_exit(void)
{
	...
	for(i=0; i<MAX_VFE_INPUT; i++) {
		if(vfe_used[i])
		{
			...
			vfe_print("platform_device_unregister[%u]\n",i);
		}
	}
