...
static void get_audio_param(void)
{
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk("[audiocodec] DAC_VOL_CTRL_SPK type err!\n");
	} else {
		dac_vol_ctrl_spk = val.val;
	}
	type = script_get_item("audio0", "DAC_VOL_CTRL_HEADPHONE", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk("[audiocodec] DAC_VOL_CTRL_HEADPHONE type err!\n");
	} else {
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk("[audiocodec] agc_used type err!\n");
	} else {
		agc_used = val.val;
	}
	type = script_get_item("audio0", "drc_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk("[audiocodec] drc_used type err!\n");
	} else {
		drc_used = val.val;
	}
...
//		codec_wr_control(0x48c, 0x7ff, 0, 0x0);
		...
		codec_wr_control(0x494, 0x7ff, 0, 0x0);
		codec_wr_control(0x498, 0xffff, 0, 0x1fb6);
