...
int axp_set_supply_status_byregulator(const char* id, int onoff)
{
    ...
    for(i = 1; i <= 2; i++)
    {
        ...
        main_hd = script_parser_fetch(main_key,"ldo_count",&ldo_count, 1);
        if(main_hd != 0)
        {
            printf("unable to get power supply by regulator [%s] %s\n",main_key,id);
    		return -1;
    	}
        for(index = 1; index <= ldo_count; index++)
        {
            sprintf(sub_key_name, "ldo%d", index);
            main_hd = script_parser_fetch(main_key,sub_key_name,(int*)(sub_key_value), sizeof(sub_key_value)/sizeof(int));
            if(main_hd != 0)
            {
                printf("unable to get power supply by regulator %s\n",id);
        		return -1;
        	}
    ...
    printf("supply regelator %s=id, pmu_type=%s_%s onoff=%d\n", id,pmu_type,vol_type,onoff );
    //para vol = 0  indcate not set vol,only open or close  voltage switch 
    return axp_set_supply_status_byname(pmu_type,vol_type,0, onoff);
}
